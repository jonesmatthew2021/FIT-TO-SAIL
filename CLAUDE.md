...
| `deploy/` | Docker Compose for **one persistent instance on its own Tailscale node** — Postgres with a volume, the backend under the `demo` profile, Caddy serving the built SPA, and a `tailscale` sidecar the SPA runs inside so nothing binds a host port; `attest.sh` over `rebuild.sh`, `reset.sh`, `backup.sh` | **running** at `https://attest.monster-mora.ts.net`, **operated entirely over ssh** (`ssh chris-hp attest update`): the box pulls from GitHub over a read-only deploy key and snapshots the portal itself. **Not `infra/`**: no cloud service anywhere in it, so it does not pre-empt ADR 0005 |
...
| `scripts/` | `dev-start.sh` / `dev-stop.sh` — the local development stack; `mobile-start.sh` — the crew app on an iOS simulator; `android-start.sh` / `android-stop.sh` — the same app on an emulator; `portal-snapshot.sh` — snapshot the Coolibah portal (real crew data) into `~/coolibah-portal`, outside the repo (runs on the Mac or, via `attest snapshot`, on the tailnet box itself) | works; see below |
