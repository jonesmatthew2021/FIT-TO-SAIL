...
///
/// **Only two of these are implemented server-side today** — `evidence.submit` and
/// `notification.read`, which predate this file. The rest are queued exactly like those two and
/// come back `rejected` with `Unsupported operation type '<kind>'`, which the row then shows as
/// a failure the crew member can retry. That is the intended state, not an oversight: the screens
/// are built, the operations are named, and `docs/handoff/mobile-crew-app-backend.md` is the list
/// of what has to exist server-side before any of them lands.
library;
